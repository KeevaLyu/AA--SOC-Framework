package CSU_Yunlu_2021.module.algorithm;

import CSU_Yunlu_2021.CSUConstants;
import CSU_Yunlu_2021.standard.Ruler;
import CSU_Yunlu_2021.world.CSUWorldHelper;
import CSU_Yunlu_2021.world.graph.GraphHelper;
import CSU_Yunlu_2021.world.graph.MyEdge;
import CSU_Yunlu_2021.world.graph.Node;
 ...
 * @Date: 03/18/2020
 */
public class ThetaStarPathPlanning extends PathPlanning {
    ...
    public ThetaStarPathPlanning(AgentInfo ai, WorldInfo wi, ScenarioInfo si, ModuleManager moduleManager, DevelopData developData) {
...
private MessageBuilding getMessageBuilding(@Nonnull MessageManager messageManager, boolean isRadio, @Nonnull EntityID senderID, byte[] data) {
    ...
    private List<EntityID> getPath(Area sourceArea, Area target) {
        ...
        boolean repeatThetaStar = !isPositionOnPreviousPath(sourceArea.getID());
        if (repeatThetaStar || repeatPlanning) {
            previousPath.clear();
        ...
        while ((maxDepth < graph.getNodeSize()) && (open.size() != 0)) {
            ...
            for (Pair<EntityID, MyEdge> neighbour : current.getNeighbourNodes()) {
                ...
                if (!closed.contains(neighbourNode.getId()) && ((neighbourMyEdge.isPassable()) || amIPoliceForce)) {
                    ...
                    int straightLineG = current.getG() + (int) Ruler.getDistance(current.getPosition(), neighbourNode.getPosition());
                    // Check if there is a straight line path
                    //if (hasStraightLinePath(current.getPosition(), neighbourNode.getPosition())) {
                    if (!open.contains(neighbourNode)) {
                        // Update neighbour node information with straight line path
                        ...
                        neighbourNode.setG(straightLineG);
                        neighbourNode.setCost(straightLineG + neighbourNode.getHeuristic());
                        ...
                        if (neighbourNode.getG() > neighbourG){
                            neighbourNode.setParent(current.getId());
                            neighbourNode.setG(straightLineG);
                            neighbourNode.setCost(neighbourNode.getHeuristic() + straightLineG);
                            neighbourNode.setDepth(current.getDepth() + 1);
                            if (neighbourNode.getDepth() > maxDepth) {
                                maxDepth = neighbourNode.getDepth();
                            }
    ...
    private boolean hasStraightLinePath(Pair<Integer, Integer> start, Pair<Integer, Integer> end) {
        for (MyEdge edge : getEdgesAlongLine(start, end)) {
            if (!edge.isPassable()) {
                return false;
            }
        }
    ...
    private List<MyEdge> getEdgesAlongLine(Pair<Integer, Integer> start, Pair<Integer, Integer> end) {
        ...
        while (true) {
            edges.addAll(getEdgesAtLocation(new Pair<>(x, y)));
            if (x == x2 && y == y2) {
                break;
            }
            int e2 = 2 * err;
    ...
    private List<MyEdge> getEdgesAtLocation(Pair<Integer, Integer> location) {
        List<MyEdge> edges = new ArrayList<>();
        Node node = graph.getNode(location);
        if (node != null) {
            for (EntityID areaId: node.getNeighbourAreaIds()) {
                edges.addAll(graph.getMyEdgesInArea(areaId, node));
//                 for (MyEdge edge : graph.getMyEdgesInArea(areaId)) {
...
//                 }
    ...
    private Node getNearestNode(Area area, Pair<Integer, Integer> XYPair) {
        ...
        for (Node node : areaNodes) {
            if (node.isPassable()) {
                boolean isLinePassable = isLinePassable(XYPair, node.getPosition());
                if (!isLinePassable) {
                    continue; // 直线不可行，跳过当前节点
                }
    ...
    // 判断两点之间的直线是否可行
    private boolean isLinePassable(Pair<Integer, Integer> point1, Pair<Integer, Integer> point2) {
        int x1 = point1.first();
        int y1 = point1.second();
        int x2 = point2.first();
        int y2 = point2.second();
        // Bresenham 算法判断直线上的所有点是否可行
        int dx = Math.abs(x2 - x1);
        int dy = Math.abs(y2 - y1);
        int sx = (x1 < x2) ? 1 : -1;
        int sy = (y1 < y2) ? 1 : -1;
        ...
        if (x1 == x2 || y1 == y2){
            while (x1 != x2 || y1 != y2) {
                Node node = graph.getNode(new Pair<>((int) x1, (int) y1));
