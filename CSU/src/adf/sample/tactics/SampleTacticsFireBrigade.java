...
public class SampleTacticsFireBrigade extends TacticsFireBrigade {
  ...
  public Action think( AgentInfo agentInfo, WorldInfo worldInfo,
    ...
    EntityID tar = this.humanDetector.calc().getTarget();
    Action act = this.actionFireRescue.setTarget( target ).calc().getAction();
    if ( action != null ) {
      this.sendActionMessage( messageManager, agent, action );
      return action;
    }
    if ( act != null ) {
       this.sendActionMessage( messageManager, agent, act );
       return act;
    }
    target = this.search.calc().getTarget();
    action = this.actionExtMove.setTarget( target ).calc().getAction();
    tar = this.search.calc().getTarget();
    act = this.actionExtMove.setTarget( tar ).calc().getAction();
    if ( action != null ) {
      this.sendActionMessage( messageManager, agent, action );
      return action;
    }
